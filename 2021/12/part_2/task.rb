...
class Task < BaseTask
  ...
  def generate_paths(map, path, current_cave, visited=Set.new, visited_twice=false)
    paths = []

    puts "Path: #{path}" if DEBUG
    puts "Current Cave: #{current_cave}" if DEBUG
    puts "Visited: #{visited}" if DEBUG
    puts "Visited Twice #{visited_twice}" if DEBUG

    visited << current_cave

    map[current_cave].each do |next_cave|
       puts "Next Cave: #{next_cave}" if DEBUG
       run_visited_twice = visited_twice

      if next_cave.downcase == next_cave && visited.include?(next_cave)
        if run_visited_twice = visited_twice
          next
        else
          puts "VISITING TWICE" if DEBUG
          run_visited_twice = true
        end
      end
      ...
      puts "Calling generate new paths" if DEBUG
      new_paths = generate_paths(map, new_path, next_cave, visited.dup, run_visited_twice ? true : false)
